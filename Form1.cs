...
namespace Курсач_Blowfish
{
    public partial class Form1 : Form
    {
        ...
        /// ulong - 64bit
        /// </summary>
        static uint[,] __Sbox = new uint[4, 256]
        {
    { // Sbox[0]
    ...
    { // Sbox[3]
        ...
        static uint[] __Keys32b = new uint[18]
        {
    ...
    0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917, 0x9216d5d9, 0x8979fb1b,
        ...
        static uint[,] _Sbox = new uint[4, 256]
       {
    { // Sbox[0]
        0xd1310ba6, 0x98dfb5ac, 0x2ffd72db, 0xd01adfb7, 0xb8e1afed, 0x6a267e96,
        0xba7c9045, 0xf12c7f99, 0x24a19947, 0xb3916cf7, 0x0801f2e2, 0x858efc16,
    ...
    { // Sbox[3]
        ...
        static uint[] _Keys32b = new uint[18]
        {
    0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0,
    0x082efa98, 0xec4e6c89, 0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c,
    0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917, 0x9216d5d9, 0x8979fb1b,
        ...
        uint join_8bits_to_32bits(ref byte[] blocks8b, int n)
        {
            uint block32b = 0;
            ...
            switch (mode)
            {
                case 'E': // Encode
                    for (byte round = 0; round < 16; ++round)
                        round_of_feistel_cipher(ref N1, ref N2, keys32b[round]);

                    swap(ref N1, ref N2); // возвращаем на места блоки
                    N2 ^= keys32b[16];   // и ксорим блоки с последними ключами 
                    N1 ^= keys32b[17];
                    break;

                case 'D': // Decode
                    for (byte round = 17; round > 1; --round)
                        round_of_feistel_cipher(ref N1, ref N2, keys32b[round]);
        ...
        void round_of_feistel_cipher(ref uint N1, ref uint N2, uint key32b)
        {
            uint temp;
            N1 ^= key32b; // левый блок ксорим с ключом 
            temp = N1;
            N1 = F(N1);   // левый блок проносим через функцию F
            N1 ^= N2;     // ксорим то что получилось с правым блоком
            N2 = temp;
        }
        uint F(uint block32b)
        {
        ...
        void key_extension(ref uint[,] Sbox, ref uint[] key, byte[] KeyByte)
        {
            ...
            byte[] init_blocks8b = new byte[8];
            for (int i = 0; i < 18; i += 2)
            {
                blowfish(ref init_blocks8b, ref init_blocks8b, key, 'E');
                key[i] = join_8bits_to_32bits(ref init_blocks8b, 0);
                key[i + 1] = join_8bits_to_32bits(ref init_blocks8b, 4);
            }
            for (int i = 0; i < 4; ++i)
        ...
        void blowfish(ref byte[] encode, ref byte[] decode, uint[] key, char mode) // mode - 'E' or 'D'
        {
            ...
            {
                uint LBlock = join_8bits_to_32bits(ref encode, i),
                    RBlock = join_8bits_to_32bits(ref encode, i + 4);
                feistel_cipher(mode, ref LBlock, ref RBlock, ref key);
                byte[] Rblock8b = new byte[4];
                byte[] Lblock8b = new byte[4];
                split_32bits_to_8bits(RBlock, ref Rblock8b);
                split_32bits_to_8bits(LBlock, ref Lblock8b);
                for (int j = 0; j < 4; j++)
                {
                    decode[i + j] = Lblock8b[j];
                    decode[i + j + 4] = Rblock8b[j];
                }
            }
        }
        ...
        private void DeCode_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                key_extension(ref _Sbox, ref _Keys32b, KeyByte);
                blowfish(ref textEbyte, ref textDbyte, _Keys32b, 'D');
            }
            catch (Exception)
            {
